...
class RunScript(BaseModel):
    ...
    action: Union[str, None] = Field(
        ...
        title='Script action',
        description='What action to take: direct_response, redirect or serve_local'
    )
    direct_response_status_code: Union[int, None] = Field(
        default=200,
        title='HTTP response status code',
        description='The HTTP status code to return')
    direct_response_mime_type: Union[str,
                     None] = Field(default='application/json',
                                   title='MIME type to return',
                                   description='MIME type of the return body')
    direct_response_body: Union[str, None] = Field(default='{}',
                                   title='Body content',
    ...
    redirect_status_code: Union[str, None] = Field(
        ...
        description=
        'Which redirect status code to use 301,302,307'
    )
    ...
    serve_local_file_path: Union[str, None] = Field(
        ...
        description=
        'Local file path for the file to serve for the request'
    )
                    ...
                    try:
                        ...
                        if ipaddress.IPv4Address(client) in network:
                            ...
                            if client_match_policies['CIDR']['hosts'] > 0:
                                if network.num_addresses < client_match_policies['CIDR']['hosts']:
                                    client_match_policies['CIDR']['hosts'] = network.num_addresses
                                    client_match_policies['CIDR']['hashes'] = [policy_hash]
                                elif network.num_addresses == client_match_policies['CIDR']['hosts']:
                                    client_match_policies['CIDR']['hashes'].append(policy_hash)
                            else:
                                client_match_policies['CIDR']['hosts'] = network.num_addresses
                                client_match_policies['CIDR']['hashes'] = [policy_hash]
                    except:
...
def get_matched_policy_hash(request, policy_hashes):
    ...
    for policy_hash in policy_hashes:
                ...
                for header in request.headers.raw:
                    if header_name.lower() == header[0].lower():
                        if header_match_value == 'any':
                            ...
                            logger.debug('header: %s matched value exactly', header_name)
                            header_match = True
                            ...
                            try:
                                ...
                                if p.match(header[1]):
                                    logger.debug('header: %s match regex', header_name)
                                    header_match = True
                                    policy_match_score = policy_match_score + 3
                            except:
                                pass
        ...
        if policy['path_re_match'] == 'ALL':
            ...
            logging.debug(
                'trying path regex match: %s for %s',
...
def apply_policy(client_ips, request, response):
    ...
    if policy:
        ...
        logger.debug("%s - matched policy: %s", request.url.path, policy_match_hash)
        script_count = len(policies[policy_match_hash]['reply_scripts'])
        for i, script in enumerate(policies[policy_match_hash]['reply_scripts']):
            script_hash = str(i)
            if script['repeat'] == 0:
                ...
                if policies[policy_match_hash]['repeated'][script_hash] <= script[
                        'repeat']:
                    logger.info(
        ...
        return Response(content='no policy matched', status_code=400, media_type='text/plain')
...
async def set_level(level: str = 'INFO'):
    if level:
        if level in logging._nameToLevel:
            ...
            raise HTTPException(status_code=400, detail='Invalid level: %s' % level)
    else:
        level = logger.getEffectiveLevel()
    return Response(content=json.dumps({ 'log_level': level }), status_code=200)


@app.get('/policies/', response_model=List[Policy], response_class=JSONResponse)
...
async def post_response(path_value: str, request: Request, response: Response):
    ...
    return apply_policy([origin_ip, forward_ip, client_host], request, response)
